...
let
  ...
  colorType = types.either types.int (types.enum [
...
in
{
  options.programs.tmenu = {
    ...
    theme = mkOption {
      type = types.nullOr (types.enum [ "catppuccin-mocha" "gruvbox-dark" "dracula" ]);
      default = null;
  ...
  config = mkIf cfg.enable {
    ...
    xdg.configFile."tmenu/config.ini" = mkIf (cfg.settings != { } || cfg.colors != { } || cfg.theme != null || cfg.menuItems != { }) {
      source = 
